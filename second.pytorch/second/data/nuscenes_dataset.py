...
class NuScenesDataset(Dataset):
    ...
    def get_sensor_data(self, query):
        ...
        if self.NumPointFeatures == 4: # default

            points = np.fromfile(
                str(lidar_path), dtype=np.float32, count=-1).reshape([-1, 5])
            # points[:, 3] /= 255  # OPERA ROSA's Points intensity is [0, 1]
            points[:, 4] = 0
        
        elif self.NumPointFeatures >= 5: # painted (x, y, z, r, c1,.., cn)
            points = np.fromfile(
                str(lidar_path), dtype=np.float32, count=-1)
            points = points.reshape([-1, self.NumPointFeatures])
            # points[:, 3] /= 255  # OPERA ROSA's Points intensity is [0, 1]
        ...
        for sweep in info["sweeps"]:
            ...
            if self.NumPointFeatures == 4: # default
                ...
                # points_sweep[:, 3] /= 255    # OPERA ROSA's Points intensity is [0, 1]
                points_sweep[:, :3] = points_sweep[:, :3] @ sweep[
                    "sweep2lidar_rotation"].T
                points_sweep[:, :3] += sweep["sweep2lidar_translation"]
                points_sweep[:, 4] = ts - sweep_ts

            elif self.NumPointFeatures >= 5:
                points_sweep = np.fromfile(
                    str(sweep["lidar_path"]), dtype=np.float32,
